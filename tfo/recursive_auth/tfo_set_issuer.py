...
def query(tup):
    ...
    results = ["{}, {}, {}".format(resolver_ip, qname, send(qname, resolver))]
...
def main():
    ...
    tcpdump_filter = "(tcp port 53 or port 853)"
    ...
    try:
        ...
        with open(parser_args.logfile, 'w') as log:
            with mp.Pool(processes=num_threads) as p:
                for result in tqdm(p.imap_unordered(query, args), total=len(args)):
                    log.write('\n'.join(result) + '\n')
